...
<head>
    ...
    <style>
        body { font-size: 0.9rem; background: #f8f9fa;  margin-top: 60px; }
...
<div class="container my-3 main-container">
    ...
    <!-- Buttons Row -->
    <div class="card p-3 mb-3 fixed-buttons">
        <div class="d-flex flex-wrap gap-2 justify-content-start">
            <button class="btn btn-secondary btn-sm" id="transactionBtn">Transaction</button>
            <button class="btn btn-info btn-sm" id="setCustomerBtn" data-bs-toggle="modal" data-bs-target="#customerModal">Set Customer</button>
            ...
            <button class="btn btn-danger btn-sm" id="voidBtn">Void</button>
            <button class="btn btn-success btn-sm" id="deliveryBtn">Delivery</button>
            <button class="btn btn-warning btn-sm" id="nextInvoiceBtn">Next Invoice #</button>
            <button class="btn btn-dark btn-sm" id="profitBtn">Profit</button>
            <button class="btn btn-outline-primary btn-sm" id="invoicesBtn">Invoices</button>
        </div>
    </div>
</div>
...
function loadState() {
    try {
        ...
        state.items.forEach(item => {
            const newRow = `
                <tr data-row-id="${item.id}" data-taxpercentage="${item.taxPercentage}" data-price-excl="${item.priceExcl}">
                    <td>${item.id}</td>
                    <td>${item.code}</td>
                    <td>${item.description}</td>
                    <td><input type="number" class="form-control form-control-sm discount-input" value="${item.discount}" min="0" max="100"></td>
    ...
    try {
        ...
        document.querySelectorAll("#itemsTable tbody tr").forEach(row => {
            row.addEventListener("dblclick", function() {
                ...
                itemCounter++;
                let newRow = `
                    <tr data-row-id="${itemCounter}" data-taxpercentage="${taxPercentage}" data-price-excl="${priceExcl}">
                        <td>${itemCounter}</td>
                        <td>${code}</td>
                        <td>${description}</td>
        ...
        document.getElementById("nextInvoiceBtn").addEventListener("click", function() {
            console.log("Next Invoice # button clicked");
            // Implement next invoice logic here
        });

        document.getElementById("profitBtn").addEventListener("click", function() {
            console.log("Profit button clicked");
            // Implement profit logic here
        });
