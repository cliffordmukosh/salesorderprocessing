...
function loadState() {
    try {
        ...
        state.items.forEach(item => {
            const newRow = `
                <tr data-row-id="${item.id}" data-taxpercentage="${item.taxPercentage}" data-price-excl="${item.priceExcl}" data-discount="${item.discount}" data-discount-type="${item.discountType}">
                    <td>${item.id}</td>
                    <td>${item.code}</td>
                    ...
                    <td class="discount-cell">${item.discountType === "percentage" ? item.discount + "%" : "KES " + item.discount}</td>
        ...
        document.getElementById("customerInfo").innerHTML = state.customerInfo || `
        ...
        document.getElementById("headerTitle").textContent = state.headerTitle || "Sales Order Processing - Sales Return";
        updateTotals();
    } catch (e) {
...
function initializeEventListeners() {
    try {
        ...
        document.getElementById("closeTransactionPanel").addEventListener("click", function() {
        ...
        document.getElementById("voidBtn").addEventListener("click", function() {
            ...
            document.getElementById("headerTitle").textContent = "Sales Order Processing - Sales Return";
            updateTotals();
            location.reload();
        });
        ...
        document.querySelectorAll("#itemsTable tbody tr").forEach(row => {
            row.addEventListener("dblclick", function() {
                ...
                let existingRow = Array.from(document.querySelectorAll("#itemsTableMain tbody tr"))
                ...
                if (existingRow) {
                    ...
                    let totalIncl = discountType === "percentage" 
                        ? totalInclBase * (1 - discount / 100) 
                        : totalInclBase - discount;

                    existingRow.querySelector(".original-price").textContent = priceExcl.toFixed(2);
                    existingRow.querySelector(".price").textContent = priceExcl.toFixed(2);
                    existingRow.querySelector(".total-excl").textContent = totalExcl.toFixed(2);
                    existingRow.querySelector(".total-incl").textContent = totalIncl.toFixed(2);
                } else {
                    ...
                    let newRow = `
                        <tr data-row-id="${itemCounter}" data-taxpercentage="${taxPercentage}" data-price-excl="${priceExcl}" data-discount="${discount}" data-discount-type="${discountType}">
                            ...
                            <td>${code}</td>
                            <td>${description}</td>
                            <td class="original-price">${priceExcl.toFixed(2)}</td>
                            <td><input type="number" class="form-control form-control-sm quantity-input" value="${quantity}" min="1"></td>
                            <td class="discount-cell">${discountType === "percentage" ? discount + "%" : "KES  " + discount}</td>
    ...
    <i class="fas fa-trash text-danger"></i>
                    ...
                    `;
                    document.querySelector("#itemsTableMain tbody").insertAdjacentHTML("afterbegin", newRow);
        ...
        document.addEventListener("click", function(e) {
            if (e.target.classList.contains("discount-cell")) {
                ...
                document.getElementById(discountType === "percentage" ? "discountPercentage" : "discountAmount").checked = true;

                // Show modal
                let discountModal = new bootstrap.Modal(document.getElementById("discountModal"));
                discountModal.show();
            }
        });
        ...
        document.getElementById("applyDiscountBtn").addEventListener("click", function() {
            ...
            let quantity = parseFloat(currentRow.querySelector(".quantity-input").value) || 1;
            ...
            currentRow.querySelector(".discount-cell").textContent = discountType === "percentage" ? discount + "%" : "KES  " + discount.toFixed(2);
            ...
            currentRow.querySelector(".original-price").textContent = priceExcl.toFixed(2);
            currentRow.querySelector(".price").textContent = priceExcl.toFixed(2);
            currentRow.querySelector(".total-excl").textContent = totalExcl.toFixed(2);
            currentRow.querySelector(".total-incl").textContent = totalIncl.toFixed(2);
        ...
        document.addEventListener("click", function(e) {
            if (e.target.classList.contains("remove-item")) {
                currentRow = e.target.closest("tr"); // Store the row to remove
                if (!currentRow) return;

                // Set item name in modal (use Description or fallback if empty)
                let itemName = currentRow.cells[2].textContent || "this item";
                document.getElementById("removeConfirmMessage").textContent = `Are you sure you want to remove "${itemName}"?`;
        ...
        document.getElementById("confirmRemoveBtn").addEventListener("click", function() {
            if (currentRow) {
                ...
                let modal = bootstrap.Modal.getInstance(document.getElementById("removeConfirmModal"));
                modal.hide();
...
if (document.querySelector("#itemsTableMain tbody").children.length === 0) {
    for (let i = 0; i < 3; i++) {
        ...
        `;
        document.querySelector("#itemsTableMain tbody").insertAdjacentHTML("beforeend", emptyRow);
    }
}
...
document.getElementById("closePropertiesBtn").addEventListener("click", function() {
    ...
    const propertiesModal = bootstrap.Modal.getInstance(document.getElementById("propertiesModal"));
